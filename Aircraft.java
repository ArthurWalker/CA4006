...
public class Aircraft implements Runnable {
	...
	public synchronized boolean checkLock(Robot robot) {
		if (getArrivalRobot() == robot.getRobotID() && Arrays.asList(robot.getAircraftTask()).contains(this.getAircraftID()) == true) {
			return true;
		}else {
			return false;
		}
...
//				System.out.println(robot.getRobotID()+" takes "+time+" miliseconds");
				...
				System.out.println("Finish current RobotID: "+getArrivalRobot()+" in "+getProcessingTime()+" at "+getFinishedTime()+" in Thread "+ Thread.currentThread().getName());
				this.lock = false;
			}catch (Exception e) {
...
//		System.out.println(nextRobot.getRobotID());
		if (getRecordedRobot()[nextRobot.getRobotID()-1]==false) {
			...
			System.out.println("Execute with next RobotID: "+nextRobotID+" at "+getArrivalTime()+" in Thread "+ Thread.currentThread().getName());
